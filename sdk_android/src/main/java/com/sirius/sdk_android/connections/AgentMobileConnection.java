 ...
 */
public class AgentMobileConnection extends BaseAgentConnection {
    ...
    public AgentMobileConnection(String serverAddress, byte[] credentials, P2PConnection p2p, int timeout) {
    ...
    public Object remoteCall(String msgType, RemoteParams params, boolean waitResponse)
        ...
        if (getTimeout() != 0) {
            expirationTime = (System.currentTimeMillis() + (getTimeout() * 1000)) / 1000;
        }
        ...
        if (waitResponse) {
            boolean success = future.waitPromise(getTimeout());
            if (success) {
    ...
    public void setup(Message context) {
       ...
       /* List<JSONObject> proxies = new ArrayList<>();
        ...
        tunnelRpc = new AddressedTunnel(channel_rpc, getConnector(), getConnector(), getP2p());
        tunnelCoprotocols = new AddressedTunnel(channel_sub_protocol, getConnector(), getConnector(), getP2p());*/
    ...
    public Message sendMessage(Message message, List<String> their_vk, String endpoint,
        ...
        if (preferAgentSide) {
            paramsBuilder.add("timeout", getTimeout());
    ...
    public List<Pair<Boolean, String>> sendMessageBatched(Message message, List<RoutingBatch> batches) throws SiriusConnectionClosed {
        ...
        try {
            ...
            for(int i=0;i<jsonArr.length();i++){
                JSONArray internalArr = (JSONArray) jsonArr.get(i);
    ...
    public Message readProtocolMessage() throws SiriusInvalidPayloadStructure {
        return this.tunnelCoprotocols.receive(getTimeout());
    }
